class Node {
  ...
  private PVector renderLocation;
  ...
  //Colour of fill
  private Color fill;
  //Colour of fill when highlighted
  private Color highlightFill;
  //Colours of outline
  private Color stroke;
  //Is this node highlighted
  private boolean isHighlighted;
  //If false, will arrange children as if it had 1 more than it actually does
  private boolean ignoreParent;
  //List of child nodes
  private ArrayList<Node> children;

  Node(PVector location, PVector renderLocation, float size, float actualSize, float outlineSize, float alignAngle, String text, Color fill, Color highlight, Color stroke, boolean ignoreParent, int childCount) {
  ...
  void setHighlighted(boolean target){
    this.isHighlighted = target;
  }
  
  boolean flipHighlighted(){
  ...
  void setHighlight(Color fill){
    this.highlightFill = fill;
  }
  
  void setHighlight(float fill){
    this.highlightFill = new Color(fill,fill,fill);
  }
  
  void setOutline(Color stroke){
    this.stroke = stroke;
  }
  
  void setOutline(float stroke){
    this.stroke = new Color(stroke,stroke,stroke);
  }
  ...
  void shallowRender() {
    ...
    fill((this.isHighlighted())?this.getHighlight():this.getFill());
  ...
  //If the specified target node is a direct child of this node it is removed
  //Returns if the node was removed or not
  ...
  //Removes the child at the specified index and returns if it was successful
  ...
  //If the specified target node is a  direct or inherited child of this node it is removed
  void deepDeleteChild(Node target) {
 
    Deque<Node> list = new ArrayDeque<Node>();
    list.add(this);
  ...
  //Attempts to delete this node and deletes all direct and inherited child nodes
  ...
  //Deletes all direct and inherited child nodes
  void deleteAllChildren(){
    Deque<Node> list = new ArrayDeque<Node>(this.children());
    this.children.clear();
