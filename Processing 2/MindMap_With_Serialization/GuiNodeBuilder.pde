class GuiNodeBuilder extends NodeBuilder<GuiNodeBuilder> {
  ...
  GuiNodeBuilder registerNodeMethod(String methodName) {
    try {
      this.action = MindMap_With_Serialization.this.getClass().getMethod(methodName, GuiNode.class, Node.class);
    }
  ...
  GuiNode build() {
    return new GuiNode(this.action, this.context, this.hintText, this.location, this.renderLocation, this.size, this.actualSize, this.outlineSize, this.alignAngle, this.text, this.fill, this.stroke, this.ignoreParent, this.childCount);
  }
}
